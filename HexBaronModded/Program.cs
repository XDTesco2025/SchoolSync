...
namespace HexBaronCS
{
    class Program
    {
        ...
        public static void SetUpDefaultGame(out Player player1, out Player player2, out HexGrid grid)
        {
            ...
            string P1PlayerName = Console.ReadLine();
            Console.WriteLine("Enter Player Twos name");
            string P2PlayerName = Console.ReadLine();
            
            player1 = new Player(P1PlayerName, 0, 20, 10, 5);
            player2 = new Player(P2PlayerName, 1, 10, 10, 5);
            grid.SetUpGridTerrain(t);
        ...
        public static void PlayGame(Player player1, Player player2, HexGrid grid)
        {
            ...
            string Command1; string Command2; string Command3;

            bool InvalidCommandPresent;
            bool chancesRemaining = true;
            ...
            while(gameOver == false)
            {
                ...
                do
                {
                    //Print Out and Get command from user
                    Console.Write("Enter command 1: "); Command1 = Console.ReadLine();
                    Console.Write("Enter command 2: "); Command2 = Console.ReadLine();
                    Console.Write("Enter command 3: "); Command3 = Console.ReadLine();

                    InvalidCommandPresent = false;

                    //Finds Invalid Commands
                    if(!CheckCommandIsValid(Command1.Split(' ').ToList())){InvalidCommandPresent = true; Console.WriteLine("Command 1 is Invalid");}
                    if(!CheckCommandIsValid(Command2.Split(' ').ToList())){InvalidCommandPresent = true; Console.WriteLine("Command 2 is Invalid");}
                    if(!CheckCommandIsValid(Command3.Split(' ').ToList())){InvalidCommandPresent = true; Console.WriteLine("Command 3 is Invalid");} 
                
                    // Deal with chances

                    if(player1Turn == true)
                    {
                        if(InvalidCommandPresent == true)
                        {
                            player1.RemoveChancesForTurns();    
                        }
                    
                        Console.WriteLine($"You have the following chances remaining: {player1.GetChancesForTurns()}/3");

                        if(player1.GetChancesForTurns() <= 0)
                        {
                            chancesRemaining = false;
                        } 
                    }
                    else
                    {
                        if(InvalidCommandPresent == true)
                        {
                            player2.RemoveChancesForTurns();    
                        }
                    
                        Console.WriteLine($"You have the following chances remaining: {player2.GetChancesForTurns()}/3");

                        if(player2.GetChancesForTurns() <= 0)
                        {
                            chancesRemaining = false;
                        }   
                    }
                } while(chancesRemaining == true && InvalidCommandPresent == true);

                if(chancesRemaining == true && InvalidCommandPresent == false)
                {           
                    int fuelChange = 0;
                    int lumberChange = 0;
                    int supplyChange = 0;
                    ...
                    items.Add(Command1); items.Add(Command2); items.Add(Command3);
                    ...
                    if (gameOver)
                    {
                        ...
                        gameOver = grid.DestroyPiecesAndCountVPs(player1, player2);
                    }
                    Console.WriteLine($"{player1.GetName()} current state - " + player1.GetStateString());
                    Console.WriteLine($"{player2.GetName()} current state - " + player2.GetStateString());
    ...
    class WizardPiece : Piece
    {
        ...
        public override int CheckMoveIsValid(int distanceBetweenTiles, string startTerrain, string endTerrain)
        {
            if (distanceBetweenTiles == 1)
            {
                // Normal move costs 1 fuel
                return 1;
            }
            else if (distanceBetweenTiles == 2 || distanceBetweenTiles == 3)
            {
                // Teleport - only once per turn, costs 5 fuel
                if (!hasTeleported)
                {
                    hasTeleported = true;
                    return 5;
                }
                return -1; // Already teleported this turn
            }
            return -1; // More than 3 squares is invalid
        }
    ...
    class HexGrid
    {
        ...
        private int ExecuteUpgradeCommand(List<string> items, int lumberAvailable)
        {
            ...
            if (!CheckPieceAndTileAreValid(tileToUse) || lumberAvailable < 5)
            {
                return -1;
            }

            string upgradeType = items[1].ToLower();

            if (upgradeType != "pbds" && upgradeType != "less" 
                && upgradeType != "wiz" && upgradeType != "scfw")
            {
                return -1;
            }
            ...
            if (upgradeType == "pbds")
            {
                newPiece = new PBDSPiece(player1Turn);
            }
            else if (upgradeType == "less")
            {
                newPiece = new LESSPiece(player1Turn);
            }
            else if (upgradeType == "wiz")
            {
                newPiece = new WizardPiece(player1Turn);
            }
            else // scfw
            {
                newPiece = new SCFWPiece(player1Turn);
            }
        ...
        public bool DestroyPiecesAndCountVPs(Player player1, Player player2)
        {
            ...
            foreach (var t in tiles)
            {
                if (t.GetPieceInTile() != null)
                {
                    ...
                    if (noOfConnections >= thePiece.GetConnectionsNeededToDestroy())
                    {
                        ...
                        {
                            player2.AddPiecesInSupply();
                        }
                        else
                        {
                            player1.AddPiecesInSupply();
                        }
                    }
                }
            }
        ...
        public void ResetWizards()
        {
            foreach (var t in tiles)
            {
                Piece p = t.GetPieceInTile();
                if (p != null && p is WizardPiece)
                {
                    ((WizardPiece)p).ResetTeleport();
                }
            }
        }
    }
    ...
    class Player
    {
        protected int piecesInSupply, fuel, VPs, lumber, chancesForTurns;
        ...
        public virtual void ModifyTileFromSupply(int n)
        {
            piecesInSupply = n;
        }

        public virtual void AddPiecesInSupply()
        {
            piecesInSupply++;
        }

        public virtual int GetChancesForTurns()
        {
            return chancesForTurns;
        }
        public virtual void RemoveChancesForTurns()
        {
            chancesForTurns--;
        }
        public virtual void ResetChancesForTurns()
        {
            chancesForTurns = 3;
        }
    }
